...
// Scene 51 — SINGULARITY SEA. Fork of scene 50 (which stays untouched — this scene
// owns copies of everything it modifies). The experiment (docs/science-boi-lift.md,
// defect_soc): the composite Gerstner band sum, read as a complex phasor field,
// contains PHASE SINGULARITIES — drifting points where the bands cancel and the
// surface is genuinely chaotic. A stateless analytic pass (singularity_mask.glslinc)
// finds them by 2×2 winding number; the mask feeds foam v2's whitecap channel, so
// the open sea foams ITSELF at its own defects — ambient whitecaps that bloom, drift
// with the swell, and occasionally reorganize in bursts. Zero new physics: we read
// structure out of a field we already compute.
//
...
public partial class SingularitySea : Node3D
{
    private const string WaterShader = "res://shaders/ameye_water_mna_51.gdshader";
    ...
    private const string Foam2KernelPath = "res://shaders/stamp/foam_update_v2_51.glslinc";
    ...
    private bool _foam1On = false;   // Ryan preset: v1 baseline off
    ...
    private float _f2Thresh = 0.0052f;   // v2's own threshold (dropped below v1's — wake foam fires while cruising)
    private float _f2Drift = 0.735f;      // v3a: swell-drift strength (0 = static v2 behaviour)

    // --- Scene 51: phase-singularity whitecap source (stateless analytic mask) ---
    private const string SingMaskPath = "res://shaders/stamp/singularity_mask.glslinc";
    private const int SingN = 512;         // MUST equal MnaGrid — the mask is texel-matched to the foam grid
    private float _singSmooth = 0.5f;      // EMA time (s): caps fade in/out over this
    private float _singSoft = 0.5f;        // graded detection width around the threshold
    private float _singProbe = 0.35f;      // winding plaquette spacing (m) — detection scale
    private Rid _singShader, _singPipe, _singTex, _singSetOut;
    private bool _singReady;
    private bool _singOn = true;
    private float _singGain = 0.09f;     // whitecap source strength at each defect
    private float _singThresh = 3.48f;   // |winding| rad to count (2π·0.64)
    private float _singFlatten = 0.63f; // sea-state dial: 0.5 ≈ lone caps · 1.0 ≈ storm foam

    // --- Scene 51 layer: eddy-core foam (winding on the swirl velocity angle) ---
    private const string EddyMaskPath = "res://shaders/stamp/eddy_mask.glslinc";
    private Rid _eddyShader, _eddyPipe, _eddyTex, _eddySetVel, _eddySetOut, _foam2SetEddy;
    private bool _eddyReady;
    private bool _eddyOn = true;
    private float _eddyGain = 0.5f;      // whitecap source at each core
    private float _eddyMinSpeed = 0.12f; // m/s gate — still water has no angle

    // --- Scene 51 layer: ambient vortex primitives (world-lattice Kirchhoff eddies) ---
    private const string VortexInjectPath = "res://shaders/stamp/vortex_inject.glslinc";
    private Rid _vinjShader, _vinjPipe, _vinjSetVel;
    private bool _vinjReady;
    private bool _avOn = true;
    private float _avStrength = 30f;     // circulation Γ (m²/s)
    private float _avSpacing = 28f;      // lattice spacing (m)
    private float _avRadius = 4f;        // core radius (m)

    // --- 51.1: von Neumann foam coarsening (science_boi foam_reactor lift) — a CPU
    // bubble list is the foam's STRUCTURE: seeded where whitecap density is high
    // (sparse async readback), drifting with the swell, and coarsening by the merge
    // rule (smallest absorbs into nearest larger, area-conserving, ages blending).
    // A tiny kernel rasterizes it to a weighted-Voronoi cell texture the water
    // shader draws as Plateau walls + aging interiors — foam you can watch grow old.
    private const string BubbleKernelPath = "res://shaders/stamp/bubble_cells.glslinc";
    private const int BubbleMax = 128;
    private const int BubbleTexN = 256;
    ...
    private Rid _bubShader, _bubPipe, _bubTex, _bubSsbo, _bubSetTex, _bubSetSsbo;
    private bool _bubReady;
    private Texture2Drd _bubDisplayTex = null!;
    private bool _bubOn = true;
    private float _bubSeedThresh = 0.3f;   // whitecap density that births a bubble
    private float _bubLife = 14f;          // s before a bubble pops
    private float _bubMergeEvery = 0.15f;  // s between coarsening merges
    private float _bubMergeT;
    private int _bubRbTick;
    private byte[]? _bubFoamData;          // latest async foam readback
    private Vector2 _bubRbOrigin;          // window mapping captured at request time
    private Vector2I _bubRbTexOff;
    private readonly Random _bubRng = new(12345);

    // --- 50.8 (built here in 51, per the no-touching-50 rule): COARSE OUTER CASCADE.
    // A second cheap MNA ring — 256 cells over 224 m (87.5 cm cells), backward-Euler
    // (numerical dissipation is invisible far away), few sweeps, every 2nd tick —
    // receiving the SAME wake forcing. Displayed only where the inner window's fade
    // ends: a pure crossfade handoff, no inter-ring physics (the flagged-hard part).
    private const int Mna2N = 256;
    private const float Mna2Window = 224f;
    private IStampSolver? _mna2Solver;
    private Texture2Drd _mna2Tex = null!;
    private Vector2 _mna2Origin = new(-Mna2Window * 0.5f, -Mna2Window * 0.5f);
    private Vector2I _mna2TexOff;
    private bool _mna2On = true;
    private int _mna2Iters = 8;
    private float _mna2Disp = 1.0f;
    private bool _mna2Phase;  // 0 = true Gerstner amplitudes (few defects) · 1 = equal amps (many)
    ...
    private const int SwirlMax = 512;      // allocation size — the sim runs on an ACTIVE SUB-GRID ≤ this
    private int _swirlN = 512;             // live resolution slider (kernels take size by push, maps by cell ratio)
    private int _swirlNPrev = 512;
    private int _swSubsteps = 1;           // temporal resolution: the full chain S× at dt/S
    private float _swTimeScale = 0.452f;       // evolution speed multiplier
    ...
    private bool _swirlOn = false;    // Ryan preset: coffee off by default
    private float _swJet = 5.6f;        // propwash strength (m/s at full throttle)
    private float _swJetRadius = 1.6f;  // m
    private float _swShed = 2.0f;       // lateral vortex shed per rad/s of turn
    private float _swDissip = 1.0f;   // velocity retention/tick
    private int _swIters = 22;          // pressure Jacobi sweeps (kept even; warm-started)
    private float _swAdvectScale = 2.1f;
    ...
    public override void _Ready()
    {
        ...
        GetViewport().Scaling3DScale = 0.49f;
    ...
    private void InitMnaSolver()
    {
        ...
        _mna2Solver = new GpuStampSolver(rd, new Vector2I(Mna2N, Mna2N), StampPath, GpuStampSolver.Mode.Rbgs, SolveBodyPath);
    ...
    private void StepSwirlRt(byte[] advPc, int iters, byte[]? vinjPc, int substeps)
    {
        if (!_swReady)
        {
            return;
        ...
        var rd = RenderingServer.GetRenderingDevice();
        uint g = (uint)((_swirlN - 1) / 8 + 1);
        float[] small = { _swirlN, _swirlN, 0f, 0f };
        ...
        for (int s = 0; s < substeps; s++)
        {
            ...
            // Ambient vortex lattice: local in-place add on the advected velocity,
            // BEFORE projection — the solve makes the eddies real fluid features.
            if (vinjPc != null && _vinjReady)
            {
                rd.ComputeListBindComputePipeline(cl, _vinjPipe);
                rd.ComputeListBindUniformSet(cl, _vinjSetVel, 0);
                rd.ComputeListSetPushConstant(cl, vinjPc, (uint)vinjPc.Length);
                rd.ComputeListDispatch(cl, g, g, 1);
                rd.ComputeListAddBarrier(cl);
            }
    ...
    // Render thread: wipe the swirl state after a live resolution change (the old
    // texel data has no meaning on the new active sub-grid mapping).
    private void ClearSwirlRt()
    {
        if (!_swReady)
        {
            return;
        ...
        if (_eddyReady)
        {
            rd.TextureClear(_eddyTex, new Color(0, 0, 0, 0), 0, 1, 0, 1);
        }
    }
    ...
    private void InitFoam(RenderingDevice rd)
    {
        ...
        // Scene 51: singularity mask (stateless — one r16f target, no history).
        _singShader = CompileKernel(rd, SingMaskPath, "sing-mask");
        if (!_singShader.IsValid)
        {
            return;   // foam2 kernel binds the mask at set 4 — without it, stay dark
        }
        _singPipe = rd.ComputePipelineCreate(_singShader);
        var tfm = new RDTextureFormat
        {
        ...
        _singTex = rd.TextureCreate(tfm, new RDTextureView(), new Godot.Collections.Array<byte[]>());
        rd.TextureClear(_singTex, new Color(0, 0, 0, 0), 0, 1, 0, 1);
        _singSetOut = MakeFoamSet(rd, _singTex, 0, _singShader);
        _foam2SetSing = MakeFoamSet(rd, _singTex, 4, _foam2Shader);
        _singReady = true;

        // Eddy-core mask (reads the swirl fluid's final velocity each tick).
        _eddyShader = CompileKernel(rd, EddyMaskPath, "eddy-mask");
        if (!_eddyShader.IsValid)
        {
            return;   // foam2 binds the eddy mask at set 5 — without it, stay dark
        }
        _eddyPipe = rd.ComputePipelineCreate(_eddyShader);
        var tfe = new RDTextureFormat
        {
            Format = RenderingDevice.DataFormat.R16Sfloat,
            TextureType = RenderingDevice.TextureType.Type2D,
            Width = SwirlMax,
        ...
        _eddyTex = rd.TextureCreate(tfe, new RDTextureView(), new Godot.Collections.Array<byte[]>());
        rd.TextureClear(_eddyTex, new Color(0, 0, 0, 0), 0, 1, 0, 1);
        _eddySetVel = MakeFoamSet(rd, _swVelA, 0, _eddyShader);
        _eddySetOut = MakeFoamSet(rd, _eddyTex, 1, _eddyShader);
        _foam2SetEddy = MakeFoamSet(rd, _eddyTex, 5, _foam2Shader);
        _eddyReady = true;

        // Ambient vortex injection (in-place local add on the swirl velocity).
        _vinjShader = CompileKernel(rd, VortexInjectPath, "vortex-inject");
        if (_vinjShader.IsValid)
        {
            _vinjPipe = rd.ComputePipelineCreate(_vinjShader);
            _vinjSetVel = MakeFoamSet(rd, _swVelB, 0, _vinjShader);
            _vinjReady = true;
        }

        // 51.1 bubble-cell rasterizer.
        _bubShader = CompileKernel(rd, BubbleKernelPath, "bubble-cells");
        if (_bubShader.IsValid)
        {
            _bubPipe = rd.ComputePipelineCreate(_bubShader);
            var tfb = new RDTextureFormat
            {
            ...
            _bubTex = rd.TextureCreate(tfb, new RDTextureView(), new Godot.Collections.Array<byte[]>());
            rd.TextureClear(_bubTex, new Color(10f, 0f, 0f, 0f), 0, 1, 0, 1);
            _bubSsbo = rd.StorageBufferCreate(BubbleMax * 16u);
            _bubSetTex = MakeFoamSet(rd, _bubTex, 0, _bubShader);
            var u = new RDUniform { UniformType = RenderingDevice.UniformType.StorageBuffer, Binding = 0 };
            u.AddId(_bubSsbo);
            _bubSetSsbo = rd.UniformSetCreate(new Godot.Collections.Array<RDUniform> { u }, _bubShader, 1);
            _bubReady = true;
        }
    ...
    // CPU mirror of the kernel's two-band orbital velocity — bubbles ride the swell.
    private Vector2 OrbitalVel(Vector2 p)
    {
        var v = Vector2.Zero;
        for (int i = 0; i < 2; i++)
        {
            var (d, k, s, st) = BandParams(i);
            float f = k * d.Dot(p) - k * s * _field.Time;
            v += d * (st * s * Mathf.Sin(f));
        }
        return v;
    }

    // Main thread, per tick: seed from the latest foam readback, age/drift/pop, and
    // run the von Neumann merge on cadence. Returns the packed SSBO data + count.
    private (byte[] data, int count) StepBubbles(float delta)
    {
        // Seed from a sparse sample of the async foam readback (RGBA16F halfs).
        if (_bubFoamData != null)
        {
            var data = _bubFoamData;
            _bubFoamData = null;
            int n = MnaGrid.X;
            for (int s = 0; s < 300 && _bubbles.Count < BubbleMax; s++)
            {
                int tx = _bubRng.Next(n);
                int ty = _bubRng.Next(n);
                int off = (ty * n + tx) * 8;
                if (off + 2 > data.Length)
                {
                    continue;
                }
                float whitecap = (float)BitConverter.ToHalf(data.AsSpan(off, 2));
                if (whitecap < _bubSeedThresh)
                {
                    continue;
                }
                int wx = ((tx - _bubRbTexOff.X) % n + n) % n;
                int wy = ((ty - _bubRbTexOff.Y) % n + n) % n;
                var world = _bubRbOrigin + new Vector2(wx + 0.5f, wy + 0.5f) * (MnaWindow / n);
                bool crowded = false;
                foreach (var b in _bubbles)
                {
                    if (b.Pos.DistanceSquaredTo(world) < 1.2f)
                    {
                        crowded = true;
                        break;
                    }
                ...
                if (!crowded)
                {
                    ...
                    {
                        Pos = world,
                        Radius = 0.3f + 0.25f * (float)_bubRng.NextDouble(),
                        Age = 0f,
                    });
                }
            }
        ...
        // Age, drift with the swell, pop the old and the out-of-window.
        var half = MnaWindow * 0.5f;
        for (int i = _bubbles.Count - 1; i >= 0; i--)
        {
            var b = _bubbles[i];
            b.Age += delta;
            b.Pos += OrbitalVel(b.Pos) * delta;
            _bubbles[i] = b;
            var rel = b.Pos - _mnaOrigin;
            if (b.Age > _bubLife || rel.X < 0f || rel.Y < 0f || rel.X > MnaWindow || rel.Y > MnaWindow)
            {
                _bubbles.RemoveAt(i);
            }
        }

        // Von Neumann coarsening: on cadence, the smallest bubble merges into its
        // nearest neighbour within reach — area-conserving, position area-weighted.
        _bubMergeT += delta;
        if (_bubMergeT >= _bubMergeEvery && _bubbles.Count >= 2)
        {
            _bubMergeT = 0f;
            int si = 0;
            for (int i = 1; i < _bubbles.Count; i++)
            {
                if (_bubbles[i].Radius < _bubbles[si].Radius)
                {
                    si = i;
                }
            }
            ...
            for (int i = 0; i < _bubbles.Count; i++)
            {
                ...
                float d = _bubbles[si].Pos.DistanceTo(_bubbles[i].Pos);
                float reach = _bubbles[si].Radius + _bubbles[i].Radius + 0.6f;
                if (d < reach && d < best)
                {
                    best = d;
                    ni = i;
            ...
            if (ni >= 0)
            {
                var small = _bubbles[si];
                var big = _bubbles[ni];
                float a1 = small.Radius * small.Radius;
                float a2 = big.Radius * big.Radius;
                var merged = new Bubble
                {
                    Pos = (small.Pos * a1 + big.Pos * a2) / (a1 + a2),
                    Radius = Mathf.Sqrt(a1 + a2),
                    Age = Mathf.Max(small.Age, big.Age),
                };
                _bubbles[ni] = merged;
                _bubbles.RemoveAt(si);
            }
        }

        // Pack for the SSBO: xy world, z radius, w age 0..1.
        int count = Math.Min(_bubbles.Count, BubbleMax);
        var packed = new float[BubbleMax * 4];
        for (int i = 0; i < count; i++)
        {
            packed[i * 4 + 0] = _bubbles[i].Pos.X;
            packed[i * 4 + 1] = _bubbles[i].Pos.Y;
            packed[i * 4 + 2] = _bubbles[i].Radius;
            packed[i * 4 + 3] = Mathf.Clamp(_bubbles[i].Age / _bubLife, 0f, 1f);
        }
        var bytes = new byte[packed.Length * sizeof(float)];
        Buffer.BlockCopy(packed, 0, bytes, 0, bytes.Length);
        return (bytes, count);
    }

    // Render thread: upload the list and rasterize the cell texture.
    private void StepBubblesRt(byte[] ssbo, byte[] pcBytes)
    {
        if (!_bubReady)
        {
            return;
        ...
        var rd = RenderingServer.GetRenderingDevice();
        rd.BufferUpdate(_bubSsbo, 0, (uint)ssbo.Length, ssbo);
        uint g = (uint)((BubbleTexN - 1) / 8 + 1);
        long cl = rd.ComputeListBegin();
        rd.ComputeListBindComputePipeline(cl, _bubPipe);
        rd.ComputeListBindUniformSet(cl, _bubSetTex, 0);
        rd.ComputeListBindUniformSet(cl, _bubSetSsbo, 1);
        rd.ComputeListSetPushConstant(cl, pcBytes, (uint)pcBytes.Length);
        rd.ComputeListDispatch(cl, g, g, 1);
        rd.ComputeListAddBarrier(cl);
        rd.ComputeListEnd();
    }

    // Render thread: request the sparse-seeding foam readback (async — no stall).
    private void RequestFoamReadbackRt(Vector2 origin, Vector2I texOff)
    {
        if (!_foam2Ready)
        {
            return;
        ...
        var rd = RenderingServer.GetRenderingDevice();
        rd.TextureGetDataAsync(_foam2State, 0, Callable.From((byte[] data) =>
        {
            _bubFoamData = data;
            _bubRbOrigin = origin;
            _bubRbTexOff = texOff;
    ...
    // Render thread: mark the swirl fluid's vortex cores for this tick.
    private void StepEddyRt(byte[] pcBytes)
    {
        if (!_eddyReady)
        {
            return;
        ...
        uint g = (uint)((_swirlN - 1) / 8 + 1);
        long cl = rd.ComputeListBegin();
        rd.ComputeListBindComputePipeline(cl, _eddyPipe);
        rd.ComputeListBindUniformSet(cl, _eddySetVel, 0);
        rd.ComputeListBindUniformSet(cl, _eddySetOut, 1);
        rd.ComputeListSetPushConstant(cl, pcBytes, (uint)pcBytes.Length);
        rd.ComputeListDispatch(cl, g, g, 1);
        rd.ComputeListAddBarrier(cl);
        rd.ComputeListEnd();
    ...
    private Rid _foam2SetSing;

    // Render thread: re-evaluate the analytic defect mask for this tick.
    private void StepSingRt(byte[] pcBytes)
    {
        if (!_singReady)
        {
            return;
        ...
        var rd = RenderingServer.GetRenderingDevice();
        uint g = (uint)((SingN - 1) / 8 + 1);
        long cl = rd.ComputeListBegin();
        rd.ComputeListBindComputePipeline(cl, _singPipe);
        rd.ComputeListBindUniformSet(cl, _singSetOut, 0);
        rd.ComputeListSetPushConstant(cl, pcBytes, (uint)pcBytes.Length);
        rd.ComputeListDispatch(cl, g, g, 1);
        rd.ComputeListAddBarrier(cl);
        rd.ComputeListEnd();
    ...
    private void StepFoamRt(byte[] fpc, byte[] fpc2)
    {
        ...
        if (_foam2Ready)
        {
            ...
            rd.ComputeListBindUniformSet(cl, _foam2SetSing, 4);
            rd.ComputeListBindUniformSet(cl, _foam2SetEddy, 5);
    ...
    private void StepMna(float delta)
    {
        ...
        // Live swirl-resolution change: reset the toroidal mapping and wipe old state
        // (old texels have no meaning on the new active sub-grid).
        if (_swirlN != _swirlNPrev)
        {
            _swirlNPrev = _swirlN;
            float nc = MnaWindow / _swirlN;
            _swTexOff = Vector2I.Zero;
            _swOrigin = new Vector2(
                Mathf.Floor(desired.X / nc) * nc,
                Mathf.Floor(desired.Y / nc) * nc);
            RenderingServer.CallOnRenderThread(Callable.From(ClearSwirlRt));
        }

        // v3b swirl window: same toroidal follow, its own (coarser) cell granularity.
        float swCell = MnaWindow / _swirlN;
        ...
        if (sdi != 0 || sdj != 0)
        {
            ...
            _swTexOff = new Vector2I(
                ((_swTexOff.X + sdi) % _swirlN + _swirlN) % _swirlN,
                ((_swTexOff.Y + sdj) % _swirlN + _swirlN) % _swirlN);
        }
        ...
        // Temporal resolution: the chain runs S× at dt/S; forcing splats scale by 1/S
        // so total per-tick momentum is unchanged. Time scale multiplies evolution speed.
        int swSub = Math.Clamp(_swSubsteps, 1, 4);
        float swDt = (1f / 60f) * _swTimeScale / swSub;
        var jetVel = new Vector2(-sfwd.X, -sfwd.Z) * (_swJet * thr / swSub);
        var shedVel = new Vector2(sright.X, sright.Z) * (-dyawRate * _swShed * thr / swSub);
        float[] swPcF =
        {
            _swirlN, _swirlN, swDt, _swDissip, swCell, _swTexOff.X, _swTexOff.Y, _swEdge,
        ...
        _bubDisplayTex.TextureRdRid = _bubReady ? _bubTex : default;
        _mna2Tex.TextureRdRid = _mna2Solver?.HeightRid ?? default;
        ...
        // Scene 51 sing mask v2 push (12 scalars + 5 bands = 32 floats). EMA alpha from
        // the smoothing-time knob; flatten lerps true amplitudes toward uniform.
        float singCell = MnaWindow / SingN;
        float singAlpha = Mathf.Clamp(delta / Mathf.Max(_singSmooth, delta), 0.02f, 1f);
        var spc = new float[32];
        spc[0] = _field.Time;
        spc[1] = singCell;
        spc[2] = _mnaOrigin.X;
        spc[3] = _mnaOrigin.Y;
        spc[4] = _singThresh;
        spc[5] = _singOn ? _singGain : 0f;
        spc[6] = _mnaTexOff.X;
        spc[7] = _mnaTexOff.Y;
        spc[8] = singAlpha;
        spc[9] = _singSoft;
        spc[10] = _singProbe;
        var bnd = new (Vector2 d, float k, float s, float st)[5];
        float ampMean = 0f;
        for (int i = 0; i < 5; i++)
        {
            bnd[i] = BandParams(i);
            ampMean += bnd[i].st / bnd[i].k;
        }
        ampMean /= 5f;
        for (int i = 0; i < 5; i++)
        {
            float amp = Mathf.Lerp(bnd[i].st / bnd[i].k, ampMean, _singFlatten);
            spc[12 + i * 4] = bnd[i].d.X * bnd[i].k;
            spc[13 + i * 4] = bnd[i].d.Y * bnd[i].k;
            spc[14 + i * 4] = bnd[i].k * bnd[i].s;
            spc[15 + i * 4] = amp;
        }
        var singBytes = new byte[spc.Length * sizeof(float)];
        Buffer.BlockCopy(spc, 0, singBytes, 0, singBytes.Length);

        // Scene 51: eddy-core mask push (8 floats) + ambient-vortex push (12 floats).
        float[] epc = { _swirlN, _swirlN, _singThresh, _eddyOn ? _eddyGain : 0f, _eddyMinSpeed, 0f, 0f, 0f };
        var eddyBytes = new byte[epc.Length * sizeof(float)];
        Buffer.BlockCopy(epc, 0, eddyBytes, 0, eddyBytes.Length);
        // 51.1: bubble structure step (CPU) + cell-raster push.
        var (bubBytes, bubCount) = StepBubbles(delta);
        float[] bpcF = { BubbleTexN, BubbleTexN, bubCount, _mnaOrigin.X, _mnaOrigin.Y, MnaWindow, 2.5f, 0f };
        var bubPc = new byte[bpcF.Length * sizeof(float)];
        Buffer.BlockCopy(bpcF, 0, bubPc, 0, bubPc.Length);
        _bubRbTick++;
        bool requestRb = _bubRbTick % 15 == 0;
        Vector2 rbOrigin = _mnaOrigin;
        Vector2I rbOff = _mnaTexOff;

        // 50.8 outer cascade: own toroidal follow (its cell granularity), same wake
        // dipole (world coords -> outer cells), BE scheme, physical wave speed matched
        // to the inner ring by scaling c with the cell ratio.
        float m2Cell = Mna2Window / Mna2N;
        int m2di = Mathf.FloorToInt((boatPos.X - Mna2Window * 0.5f - _mna2Origin.X) / m2Cell);
        int m2dj = Mathf.FloorToInt((boatPos.Z - Mna2Window * 0.5f - _mna2Origin.Y) / m2Cell);
        if (m2di != 0 || m2dj != 0)
        {
            _mna2Origin += new Vector2(m2di * m2Cell, m2dj * m2Cell);
            _mna2TexOff = new Vector2I(
                ((_mna2TexOff.X + m2di) % Mna2N + Mna2N) % Mna2N,
                ((_mna2TexOff.Y + m2dj) % Mna2N + Mna2N) % Mna2N);
            _mat.SetShaderParameter("mna2_origin", _mna2Origin);
            _mat.SetShaderParameter("mna2_tex_offset", new Vector2(
                (float)_mna2TexOff.X / Mna2N, (float)_mna2TexOff.Y / Mna2N));
        }
        _mna2Phase = !_mna2Phase;
        bool m2Run = _mna2On && _mna2Phase;
        byte[]? m2Bytes = null;
        int m2Iters = _mna2Iters;
        if (m2Run)
        {
            var p2 = new List<Vector4>();
            float sr2 = Mathf.Abs(_boat.CurrentSpeed) / Mathf.Max(_boat.MaxSpeed, 0.01f);
            if (sr2 > 0.02f)
            {
                Vector3 f2d = -_boat.GlobalTransform.Basis.Z;
                f2d.Y = 0f;
                f2d = f2d.Normalized();
                Vector3 r2d = new(-f2d.Z, 0f, f2d.X);
                Vector3 bp2 = boatPos + r2d * _wakeSideOffset;
                float w2 = _wakeStrength * sr2 * sr2;
                void Add2(Vector3 world, float radius, float w)
                {
                    if (p2.Count >= 4) { return; }
                    float mm = (_spongeWidth + radius) / Mna2N;
                    var uv2 = new Vector2(world.X - _mna2Origin.X, world.Z - _mna2Origin.Y) / Mna2Window;
                    if (uv2.X < mm || uv2.X > 1f - mm || uv2.Y < mm || uv2.Y > 1f - mm) { return; }
                    p2.Add(new Vector4(uv2.X * Mna2N, uv2.Y * Mna2N, radius, w));
                }
                if (_bowPokeOn)
                {
                    Vector3 bow2 = bp2 + PokeDir(f2d, r2d, _bowAngle) * _bowDist;
                    Vector3 half2 = r2d * (_bowSpread * 0.5f);
                    Add2(bow2 - half2, _wakeRadius, w2 * _bowScale);
                    Add2(bow2 + half2, _wakeRadius, w2 * _bowScale);
                }
                Add2(bp2 + PokeDir(-f2d, r2d, _sternAngle) * _sternDist, _wakeRadius, w2 * _sternScale);
            }
            float cRatio = (MnaWindow / MnaGrid.X) / m2Cell;   // physical wave-speed match
            float sdt2 = _mnaDt;
            float beta2 = sdt2 * sdt2 * _mnaC * cRatio * _mnaC * cRatio;
            var m2pc = new float[28];
            m2pc[0] = Mna2N;
            m2pc[1] = Mna2N;
            m2pc[2] = beta2;
            m2pc[3] = _mnaDamping * sdt2 * 0.5f;
            m2pc[4] = _mnaLeak;
            m2pc[5] = 0f;   // backward-Euler out here: dissipation is invisible far away
            m2pc[6] = _spongeWidth;
            m2pc[7] = _spongeDamp;
            m2pc[8] = _mna2TexOff.X;
            m2pc[9] = _mna2TexOff.Y;
            for (int i = 0; i < p2.Count && i < 4; i++)
            {
                m2pc[12 + i * 4] = p2[i].X;
                m2pc[13 + i * 4] = p2[i].Y;
                m2pc[14 + i * 4] = p2[i].Z;
            ...
            m2Bytes = new byte[m2pc.Length * sizeof(float)];
            Buffer.BlockCopy(m2pc, 0, m2Bytes, 0, m2Bytes.Length);
        }
        ...
        if (_avOn)
        {
            float[] vpc =
            {
                _swirlN, _swirlN, swCell, _avSpacing,
                _swOrigin.X, _swOrigin.Y, _swTexOff.X, _swTexOff.Y,
                _avStrength, _avRadius, swDt, 0f,
            };
            vinjBytes = new byte[vpc.Length * sizeof(float)];
            Buffer.BlockCopy(vpc, 0, vinjBytes, 0, vinjBytes.Length);
        }
        RenderingServer.CallOnRenderThread(Callable.From(() =>
        {
            ...
            if (_swirlOn)
            {
                StepSwirlRt(swBytes, swIters, vinjBytes, swSub);   // fresh velocity before the foam samples it
            }
            StepEddyRt(eddyBytes);               // cores of the fresh velocity
            StepSingRt(singBytes);               // fresh defect mask before the foam samples it
            if (m2Bytes != null)
            {
                _mna2Solver?.Step(m2Bytes, m2Iters, false);
            }
            StepFoamRt(fbytes, f2bytes);
            StepBubblesRt(bubBytes, bubPc);
            if (requestRb)
            {
                RequestFoamReadbackRt(rbOrigin, rbOff);
            }
            ReadbackHeightsRt();
        }));
    ...
    public override void _ExitTree()
    {
        ...
        if (_mna2Tex != null)
        {
            _mna2Tex.TextureRdRid = default;
        }
        if (_bubDisplayTex != null)
        {
            _bubDisplayTex.TextureRdRid = default;
        }
        RenderingServer.CallOnRenderThread(Callable.From(() =>
        {
            ...
            {
                _singSetOut, _foam2SetSing,   // sets first: they die with their shader/texture
                _eddySetVel, _eddySetOut, _foam2SetEddy, _vinjSetVel, _bubSetTex, _bubSetSsbo,
                ...
                _singTex, _singShader, _eddyTex, _eddyShader, _vinjShader, _bubTex, _bubSsbo, _bubShader,
            })
            {
                if (r.IsValid)
                {
    ...
    private void BuildWater()
    {
        ...
        _bubDisplayTex = new Texture2Drd();
        _mna2Tex = new Texture2Drd();
        ...
        _mat.SetShaderParameter("bubble_tex", _bubDisplayTex);
        _mat.SetShaderParameter("mna2_tex", _mna2Tex);
        _mat.SetShaderParameter("mna2_origin", _mna2Origin);
        _mat.SetShaderParameter("mna2_window", Mna2Window);
        _mat.SetShaderParameter("foam1_enabled", _foam1On);
    ...
    {
        var ui = new DemoUI(this, "51 · Singularity sea — whitecaps from phase defects",
            "Scene 50's full stack, plus THE EXPERIMENT: the Gerstner band sum contains drifting phase singularities — points where the bands cancel and the surface is chaotic. A stateless winding-number pass finds them and feeds foam v2's whitecap channel, so the open sea foams ITSELF. Sing knobs at the bottom.");
        ...
        ui.AddSlider("Render scale (3D)", 0.4f, 1.0f, 0.49f, v => GetViewport().Scaling3DScale = v);
        ...
        ui.AddSlider("Swirl pressure iters", 4, 60, _swIters, v => _swIters = (int)v);
        ui.AddSlider("Swirl resolution (cells)", 64f, 512f, _swirlN, v => _swirlN = ((int)v) & ~7);
        ui.AddSlider("Swirl substeps (temporal res)", 1, 4, _swSubsteps, v => _swSubsteps = (int)v);
        ui.AddSlider("Swirl time scale ×", 0.2f, 3f, _swTimeScale, v => _swTimeScale = v);
        ui.AddToggle("Outer cascade (50.8)", _mna2On, v =>
        {
            _mna2On = v;
            _mat.SetShaderParameter("mna2_enabled", v);
        });
        ui.AddSlider("Outer sweeps", 2, 20, _mna2Iters, v => _mna2Iters = (int)v);
        ui.AddSlider("Outer displacement ×", 0f, 3f, _mna2Disp, v => _mat.SetShaderParameter("mna2_disp", v));
        ui.AddToggle("Singularity whitecaps (51)", _singOn, v => _singOn = v);
        ui.AddSlider("Sing whitecap gain", 0f, 3f, _singGain, v => _singGain = v);
        ui.AddSlider("Sing winding thresh (rad)", 2f, 6f, _singThresh, v => _singThresh = v);
        ui.AddSlider("Sing amp flatten (defect density)", 0f, 1f, _singFlatten, v => _singFlatten = v);
        ui.AddSlider("Sing smoothing (s)", 0.05f, 3f, _singSmooth, v => _singSmooth = v);
        ui.AddSlider("Sing softness (graded detect)", 0f, 1f, _singSoft, v => _singSoft = v);
        ui.AddSlider("Sing detect size (m)", 0.16f, 2f, _singProbe, v => _singProbe = v);
        ui.AddToggle("Eddy-core foam (51)", _eddyOn, v => _eddyOn = v);
        ui.AddSlider("Eddy foam gain", 0f, 3f, _eddyGain, v => _eddyGain = v);
        ui.AddSlider("Eddy min speed (m/s)", 0.02f, 0.6f, _eddyMinSpeed, v => _eddyMinSpeed = v);
        ui.AddToggle("Ambient vortices (51)", _avOn, v => _avOn = v);
        ui.AddSlider("Vortex strength Γ", 0f, 100f, _avStrength, v => _avStrength = v);
        ui.AddSlider("Vortex spacing (m)", 10f, 60f, _avSpacing, v => _avSpacing = v);
        ui.AddSlider("Vortex core radius (m)", 1f, 10f, _avRadius, v => _avRadius = v);
        ui.AddToggle("Foam bubbles — von Neumann (51.1)", _bubOn, v =>
        {
            _bubOn = v;
            _mat.SetShaderParameter("foam_bubbles", v);
        });
        ui.AddSlider("Bubble seed thresh", 0.05f, 1f, _bubSeedThresh, v => _bubSeedThresh = v);
        ui.AddSlider("Bubble life (s)", 3f, 40f, _bubLife, v => _bubLife = v);
        ui.AddSlider("Bubble merge cadence (s)", 0.03f, 1f, _bubMergeEvery, v => _bubMergeEvery = v);
        ui.AddSlider("Bubble wall width (m)", 0.05f, 0.6f, 0.35f, v => _mat.SetShaderParameter("bubble_wall_w", v));
        ui.AddSlider("Bubble wall bright", 0f, 1f, 0.7f, v => _mat.SetShaderParameter("bubble_wall_bright", v));
        ui.AddSlider("Bubble age dim", 0f, 1f, 0.5f, v => _mat.SetShaderParameter("bubble_age_dim", v));
        ...
        ui.AddSlider("v2 opacity (max over water)", 0f, 1f, 1.0f, v => _mat.SetShaderParameter("foam2_opacity", v));
        ui.AddSlider("v2 softness (alpha ramp)", 0.05f, 1.5f, 0.4f, v => _mat.SetShaderParameter("foam2_ramp", v));
    }
}
