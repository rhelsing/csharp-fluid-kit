...
// Scene 52 — Scene 50 CLONE + BATTY variational cut-cell shore boundaries
// (isolated: cloned stamp, shared solver untouched). The wave operator's per-face
// conductance is weighted by the sub-grid OPEN FRACTION of each cell face (from the
// islands' analytic waterline circles, baked into the stamp as constants) — Batty/
// Bertails/Bridson 2007's SPD weighted-Laplacian idea in one stamp line. Wakes now
// REFLECT off beaches; a skerry sits near spawn so it shows immediately. Panel A/B:
// Off / Smooth (sub-grid) / Binary (staircase).
// Original scene 50 header follows:
// Fork of scene 16 (BoatComposite: boat +
...
public partial class BoatBatty : Node3D
{
    ...
    private const string StampPath = "res://shaders/stamp/stamp_wave_batty.glslinc";

    private const string SolveBodyPath = "res://shaders/stamp/solve_rbgs_wrap.glslinc";

    // Batty cut-cell shore boundaries: 0 = off, 1 = smooth sub-grid fractions, 2 = binary
    private int _battyMode = 1;
    private float _battySoft = 1.0f;   // fraction transition width (cells)
    ...
    {   // first poke lands beside the skerry so auto-poke demos the shore reflection
        new(0.475f, 0.225f), new(0.65f, 0.6f), new(0.5f, 0.5f),
    ...
    private void StepMna(float delta)
    {
        ...
        else if (_mnaAutoPoke)
        {
            ...
            if (_mnaPokeT >= MnaPokeInterval)
            {
                ...
                pokes.Add(new Vector4(uv.X * MnaGrid.X + 6f, uv.Y * MnaGrid.Y, _mnaPokeRadius, _mnaPokeStrength));   // balanced pair: zero net volume, no DC pump
            }
        }
        ...
        // Push layout = stamp_wave_batty.glslinc Params: 16-float header + 4 vec4 drops = 128B.
        var pc = new float[32];
        pc[0] = MnaGrid.X;
        pc[1] = MnaGrid.Y;
        pc[2] = beta;
        ...
        if (substeps > 1)
        {
            ...
            Array.Clear(pc2, 16, 16);   // sub-ticks 2..S run force-free
    ...
    private void BuildIslands()
    {
        ...
        float[][] specs =
        {
            ...
            new[] { -12f, -22f, 12f, 10f, 1.5f },   // skerry near spawn (camera-left): reflections on screen fast
        };
    ...
    {
        var ui = new DemoUI(this, "52 · Batty shore boundaries (50 clone)",
        ...
        ui.AddOptions("Batty · shore boundary", new[] { "Off (waves ignore land)", "Smooth (sub-grid fractions)", "Binary (staircase A/B)" },
            _battyMode, i => _battyMode = i);
        ui.AddSlider("Batty · shore softness (cells)", 0.25f, 4.0f, _battySoft, v => _battySoft = v);
