...
// LIQUID variant of FluidSim3D — a FORK. FluidSim3D stays exactly as it is and every scene
// using it (09/10/11/12/17/32) is untouched.
//
// Three differences, and each is the difference between smoke and a body of liquid:
//
//  1. FREE SURFACE. f3_pressure_free puts p = 0 at air cells. A sealed box cannot hold up a
//     column that has a top — that is why a liquid in the smoke solver collapses and drains no
//     matter how many pressure iterations you throw at it. Needs the density field bound to the
//     pressure solve (set 3), which is the whole reason this is a fork and not a flag.
//
//  2. DIRECTIONAL GRAVITY + A MOVABLE CONTAINER. f3_add_gravity takes g as a vector in the
//     FLUID's frame, so tilting the vessel makes it run downhill, and an accelerating vessel
//     adds the -a pseudo-force that makes it pile against a wall.
//
//  3. DENSITY, NOT DYE. The scalar field is rho. Buoyancy is (rho - ambient)*g, so water, oil
//     and fog are ONE field at different values and they stratify by themselves.
//
...
public sealed class LiquidSim3D
{
    ...
    private Rid _jacAB0, _jacAB1, _jacAB2, _jacAB3, _jacBA0, _jacBA1, _jacBA2, _jacBA3;
    ...
    public void EnableMultigrid(int betaExp = 2)
    {
        ...
        if (!_mg.Ready) { GD.PushError("[LiquidSim3D] multigrid pressure path failed to init"); }
    }

    public LiquidSim3D(RenderingDevice rd, Vector3I grid, bool extras = false)
    {
        _rd = rd;
        _grid = grid;
        ...
        _sAdd = Compile("f3_add_gravity"); _sAdvV = Compile("f3_advect_vel");
        _sDiv = Compile("f3_divergence"); _sJac = Compile("f3_pressure_free");
        ...
        _jacAB0 = Set(_pA, _sJac, 0); _jacAB1 = Set(_div, _sJac, 1); _jacAB2 = Set(_pB, _sJac, 2); _jacAB3 = Set(_dyeA, _sJac, 3);
        _jacBA0 = Set(_pB, _sJac, 0); _jacBA1 = Set(_div, _sJac, 1); _jacBA2 = Set(_pA, _sJac, 2); _jacBA3 = Set(_dyeA, _sJac, 3);
        ...
        byte[][]? extraAdds = null, byte[]? confinePc = null)
    {
        ...
        // Multigrid is NOT wired here: MgDeepSolver3D takes two external textures and the free
        // surface needs a third. Jacobi-only is also the aesthetic verdict from scene 32 — an
        // under-converged projection cushions the flow into billows.
        if (false && UseMultigrid && _mg is { Ready: true })
        {
        ...
        else
        {
            ...
            {
                if (k % 2 == 0) { Bind(cl, _pJac, simPc, (_jacAB0, 0u), (_jacAB1, 1u), (_jacAB2, 2u), (_jacAB3, 3u)); }
                else { Bind(cl, _pJac, simPc, (_jacBA0, 0u), (_jacBA1, 1u), (_jacBA2, 2u), (_jacBA3, 3u)); }
                _rd.ComputeListAddBarrier(cl);
            }
    ...
    public void Free()
    {
        ...
        foreach (var s in new[]
        {
            ...
            _jacAB0, _jacAB1, _jacAB2, _jacAB3, _jacBA0, _jacBA1, _jacBA2, _jacBA3, _grad0, _grad1, _advD0, _advD1, _advD2,
