...
// deterministic math.
//
// Grid convention (identical to shorewaves): domain 30.4×30.4 m, 608² cells, dx = 0.05.
// Corner (i,j) at world (i·dx, j·dx), row-major corner index j*609+i. Cell (i,j) center
// at ((i+.5)dx, (j+.5)dx) → bottom texel (i,j).
public static class BeachScenario
{
    ...
    // GDScript-style smoothstep (clamped Hermite) — reproduced exactly so the bed
    // matches shorewaves cell-for-cell.
    ...
    public static float BeachBase(float x, float z)
    {
        ...
        return b + 0.05f * Mathf.Sin(z * 0.35f + 1.7f) * Smoothstep(8.0f, 22.0f, x);
    }

    // 609×609 corner heights.
    public static float[] BuildCorners()
    {
        var h = new float[Corners * Corners];
        for (int j = 0; j < Corners; j++)
        {
            float z = j * Dx;
            float sz = 0.05f * Mathf.Sin(z * 0.35f + 1.7f);
            int baseIdx = j * Corners;
            for (int i = 0; i < Corners; i++)
            {
                float x = i * Dx;
                float b = x <= 6.0f ? -1.2f : -1.2f + (x - 6.0f) / 12.0f;
                b += sz * Smoothstep(8.0f, 22.0f, x);
                h[baseIdx + i] = b;
            }
        }
        return h;
    ...
    public static float[] BuildBottomFloats(float[] corners)
    {
        int g = Grid, c = Corners;
    ...
    // Visual terrain: 609² vertex grid, indexed triangles, analytic smooth normals from
    // the heightfield, UV1 = world_xz / 30.4, mikktspace tangents.
    public static ArrayMesh BuildTerrainMesh(float[] corners)
    {
        int c = Corners, g = Grid;
